    ...
    return (
        <form action={googleLogOut} className={`inline-block ${className}`}>
            <button
                ...
                name="action"
                value="google"
                className="px-4 py-2 border border-[#2E3033] bg-[#1A1B1D] hover:bg-[#242628] text-[#F2F1ED] text-sm font-medium rounded-md transition-all flex items-center gap-2 shadow-xs cursor-pointer"
            >
                {showIcon && <GoogleIcon className="w-4 h-4" />}
                <span>{label || "Signing Out with Google"}</span>
            </button>
